...
class article {
    ...
    function listArticle() {
        ...
        $rsTotal = $db->query_first("SELECT count(n.id) as total FROM news n WHERE 1 AND n.active=1");
        $limit = NUM_ROW_PER_PAGE;
        $totalPage = ceil($rsTotal['total'] / $limit);
        $start = (($page * $limit) - $limit);
        $sql = "SELECT n.* FROM news n WHERE 1 AND n.active=1 ORDER BY n.`create_date` DESC LIMIT $start,$limit";
        $rs = $db->query($sql);
        return array("rs" => $rs, "page" => $page, "totalPage" => $totalPage);
    }

    function detailArticle() {
        global $db, $blogParams;
        $link = $blogParams[1];
        $db->query("UPDATE news SET view_count=view_count+1 WHERE title_link='$link'");
        $rs = $db->query_first("SELECT * FROM news WHERE 1 AND title_link='$link'");
        return $rs;
    }
